...
- 切歌时先等待播放状态稳定；用户正在输入或操作时延后发送 QQ 音乐快捷键，避免打断当前输入。
...
qqMusicLyrics.userIdleGuardMs=1500
qqMusicLyrics.playbackSettleMs=1200
...
- `qqMusicLyrics.userIdleGuardMs`：最近有键盘或鼠标操作时不发送 QQ 音乐快捷键，默认 `1500` 毫秒。
- `qqMusicLyrics.playbackSettleMs`：播放状态切换后等待桌面歌词状态稳定的时间，默认 `1200` 毫秒。
